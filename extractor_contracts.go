...
func main() {
	...
	suicidedContracts_File := "./suicidedContracts.result"
	//suicidedContracts_File := "./temp_suicidedContracts.result"
	file2, err := os.Open(suicidedContracts_File)
	if err != nil {
		fmt.Println("Error opening log file:", err)
		return
	}
	...
	for scanner.Scan() {
		...
		if line != "" {
			suicidedContracts[line] = true
		}
	}
	fmt.Println("Suicided contracts num ", len(suicidedContracts))
